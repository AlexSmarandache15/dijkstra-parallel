	...
	const int& source_node)
{
	...
	no_of_verticles = graph_adjacency_matrix.size();
	visited.resize(no_of_verticles, false);
	distances.resize(no_of_verticles, INFINITE);
}
...
void DijkstraSequential::solve()
{
	...
	for (int loop_index = 0; loop_index < no_of_verticles - 1; loop_index++) {
